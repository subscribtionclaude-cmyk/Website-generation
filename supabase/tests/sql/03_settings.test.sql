...
select tests.assert_equal((select count(*)::int from public.site_settings), 8, 'anon reads the 8 public settings');
...
select tests.assert_equal((select count(*)::int from public.site_settings), 8, 'customers read only public settings');
