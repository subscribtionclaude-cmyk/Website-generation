...
select tests.assert_equal((select count(*)::int from public.site_settings), 14, 'anon reads the 14 public settings');
...
select tests.assert_equal((select count(*)::int from public.site_settings), 14, 'customers read only public settings');
