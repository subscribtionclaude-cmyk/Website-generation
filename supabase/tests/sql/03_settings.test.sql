...
select tests.assert_equal((select count(*)::int from public.site_settings), 12, 'anon reads the 12 public settings');
...
select tests.assert_equal((select count(*)::int from public.site_settings), 12, 'customers read only public settings');
