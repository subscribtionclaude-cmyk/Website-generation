...
select tests.assert_equal((select count(*)::int from public.site_settings), 18, 'anon reads the 18 public settings');
...
select tests.assert_equal((select count(*)::int from public.site_settings), 18, 'customers read only public settings');
